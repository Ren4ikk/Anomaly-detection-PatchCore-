...
class TestComputePRO:
    ...
    def _make_data(self, H=16, W=16, n=4):
        ...
        Важно: карты НЕ бинарные (0.9 / 0.1, а не 1.0 / 0.0).
        _compute_pro строит пороги как linspace(min, max).
        При бинарных картах (0.0 и 1.0) единственный порог, при котором
        аномальные пиксели детектируются — это ровно 1.0, но тогда же
        нормальные пиксели со значением 0.0 не детектируются → FPR=0,
        и интеграл по FPR∈[0, 0.3] вырождается в 0. Используем значения
        с зазором, чтобы пороги давали ненулевой FPR-диапазон.
        """
        gt_masks = np.zeros((n, H, W), dtype=np.uint8)
        gt_masks[:, :H//2, :W//2] = 1  # квадрант — аномалия

        # Аномальные пиксели: 0.9, нормальные: 0.1 — хороший зазор для порогов
        anomaly_maps = np.full((n, H, W), 0.1, dtype=np.float32)
